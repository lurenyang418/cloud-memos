...
import { Check, Copy, KeyRound, Link2, Shield, UserX } from "lucide-react";
...
interface InstanceSettings { contactLabel: string; contactUrl: string }

function ContactSettingsForm({ initial }: { initial: InstanceSettings }) {
  ...
  const save = useMutation({
    mutationFn: () => api<InstanceSettings>("/api/v1/admin/settings", { method: "PATCH", body: JSON.stringify({ contactLabel, contactUrl }) }),
  ...
  return <form className="form-stack" onSubmit={submit}>
    ...
    {saved && <div className="success-panel" role="status">公开首页的联系入口已更新。</div>}
    <SubmitButton pending={save.isPending}>保存公开联系入口</SubmitButton>
  </form>;
}
...
export function AdminPage() {
  ...
  return (
    <div className="page-column wide-page">
      ...
      <section className="panel"><div className="panel-heading"><Link2 /><div><h2>公开联系入口</h2><p>匿名访客可通过这个链接联系站长申请邀请；不会公开管理员登录邮箱。</p></div></div>{settings.isPending ? <div className="loading-panel"><span>加载中</span></div> : settings.isError ? <FormError error={settings.error.message} /> : <ContactSettingsForm key={`${settings.data.contactLabel}:${settings.data.contactUrl}`} initial={settings.data} />}</section>
